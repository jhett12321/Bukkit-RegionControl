...
public class Config {
    ...
    public String getDefaultFaction() {
        Set<String> factions = mainconfig.getConfigurationSection("factions")
                .getKeys(false);
    ...
    public void reloadDataFile() {
        dataFile = new File(RegionControl.plugin.getDataFolder(),
                "data/data.yml");
    ...
    public void reloadMainConfig() {
        mainconfigfile = new File(RegionControl.plugin.getDataFolder(),
                "config.yml");
    ...
    public void saveAll() {
        ...
        Collection<CapturableRegion> regions = ServerLogic.capturableRegions
                .values();
        ...
        for (CapturableRegion region : regions) {
            if (!region.isSpawnRegion()) {
                ...
                int configInfluence = region.getInfluenceMap()
                        .get(influenceOwner).intValue();
                ...
                dataConfig.set("worlds." + configWorld + ".regions." + configId
                        + ".influenceowner", configInfluenceOwner);
                dataConfig.set("worlds." + configWorld + ".regions." + configId
                        + ".influence", configInfluence);
                dataConfig.set("worlds." + configWorld + ".regions." + configId
                        + ".owner", configOwner);
                ...
                for (ControlPoint controlPoint : controlPoints) {
                    Faction controlPointInfluenceOwner = controlPoint
                            .getInfluenceOwner();
                    String configControlPointInfluenceOwner = controlPointInfluenceOwner
                            .getName();
                    int configControlPointInfluence = controlPoint
                            .getInfluenceMap().get(controlPointInfluenceOwner)
                            .intValue();
                    String configControlPointId = controlPoint.getIdentifier();
                    String configControlPointOwner = controlPoint.getOwner()
                            .getName();

                    dataConfig.set("worlds." + configWorld + ".regions."
                            + configId + ".controlpoints."
                            + configControlPointId + ".influenceowner",
                            configControlPointInfluenceOwner);
                    dataConfig.set("worlds." + configWorld + ".regions."
                            + configId + ".controlpoints."
                            + configControlPointId + ".influence",
                            configControlPointInfluence);
                    dataConfig.set("worlds." + configWorld + ".regions."
                            + configId + ".controlpoints."
                            + configControlPointId + ".owner",
                            configControlPointOwner);
                }
            } else {
                String configOwner = region.getOwner().getName();
                String configWorld = region.getWorld().getName();
        ...
        if (saveDataFile() && saveMainConfig()) {
            ...
            RegionControl.plugin
                    .getLogger()
                    .severe("Save Failed. Please check your plugin directory has write permissions.");
        }
    }
    ...
    public Boolean saveDataFile() {
        ...
        try {
            ...
            RegionControl.plugin.getLogger().log(Level.SEVERE,
                    "Could not save data config", ex);
            return false;
        }
        return true;
    }
